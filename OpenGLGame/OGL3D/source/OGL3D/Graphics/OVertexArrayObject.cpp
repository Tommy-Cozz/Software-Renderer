...
OVertexArrayObject::OVertexArrayObject(const OVertexBufferDesc& vbDesc)
{
	...
	if (!vbDesc.vertextSize) OGL3D_ERROR("OVertexArrayObject | vertextSize is NULL");
	...
	glGenBuffers(1, &m_vertexBufferId);   //Allows to generate multiple bufferes by passing the params of nubmer of buffers and the array of all generated buffers
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexBufferId);
	glBufferData(GL_ARRAY_BUFFER, vbDesc.vertextSize * vbDesc.listSize, vbDesc.verticesList, GL_STATIC_DRAW);
	...
	glBindVertexArray(0); //This is now storing the data to be stored inside the vertex array in order to be used later when creating
...
OVertexArrayObject::OVertexArrayObject(const OVertexBufferDesc& vbDesc, const OIndexBufferDesc& ibDesc):OVertexArrayObject(vbDesc)
{
	...
	glGenBuffers(1, &m_elementBufferId);   //Allows to generate multiple bufferes by passing the params of nubmer of buffers and the array of all generated buffers
...
{
	return m_vertexBufferData.vertextSize; 
}
